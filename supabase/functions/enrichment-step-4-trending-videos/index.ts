...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (suspiciousVideos.length > 0) {
      console.warn(`[Step 4: Trending Videos] ⚠️ Found ${suspiciousVideos.length} videos with suspicious upload dates (within last hour):`)
      suspiciousVideos.forEach((video) => {
        console.warn(`  - Video ${video.youtube_video_id}: "${video.title}" - upload_date: ${video.upload_date}`)
      })
      console.warn('[Step 4: Trending Videos] These videos may have missing/invalid YouTube API date data')
    }
