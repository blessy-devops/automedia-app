-- fix-production-video-details-rpc-v2.sql
...
-- A coluna covering_images tem default '{"images": []}'::jsonb (objeto)
-- Mas o código estava tratando como se fosse um array direto
--
-- CORREÇÃO:
-- 1. Verifica se covering_images é objeto com propriedade 'images'
-- 2. Se for, acessa covering_images->'images'
-- 3. Se for array direto (legado), usa covering_images diretamente
-- 4. Caso contrário, retorna valores padrão
...
DO $$
BEGIN
  RAISE NOTICE '✅ Fixed get_production_video_details function (v2)';
  RAISE NOTICE '   - Handles covering_images as object {"images": [...]} (default format)';
  RAISE NOTICE '   - Handles covering_images as array [...] (legacy format)';
  RAISE NOTICE '   - Safely checks jsonb_typeof before calling jsonb_array_length()';
