...
export async function startChannelBenchmark(channelId: string) {
  try {
    ...
    // Step 1: Create enrichment job using Admin Client (bypasses RLS for system operations)
    ...
    // Step 3: Invoke Edge Function to start the pipeline (fire and forget - don't wait)
    console.log(`[Benchmark] Invoking enrichment-pipeline-starter for task #${task.id}, channel: ${sanitizedChannelId}`)

    // Fire and forget - invoke asynchronously without blocking
    adminClient.functions.invoke(
      ...
      if (invokeError) {
        console.error('[Benchmark] Error invoking Edge Function:', {
          error: invokeError,
          message: invokeError.message,
          taskId: task.id,
        })
      } else {
        console.log(`[Benchmark] Edge Function invoked successfully for task #${task.id}`, data)
      }
    }).catch((invokeError) => {
      console.error('[Benchmark] Exception invoking Edge Function:', {
...
/**
 * Server Action: Get Enrichment Task Status
 *
 * Fetches the current status of an enrichment task by ID.
 * Uses admin client to bypass RLS, allowing the UI to monitor progress.
 *
 * @param taskId - The ID of the enrichment task to fetch
 * @returns Task data or error
 */
export async function getEnrichmentTaskStatus(taskId: number) {
  try {
    console.log(`[Benchmark] Fetching status for task #${taskId}`)
    ...
    if (error) {
      console.error('[Benchmark] Error fetching task status:', error)
      return {
        success: false,
    ...
    console.log(`[Benchmark] Task #${taskId} status: ${data.overall_status}`)
    ...
    console.error('[Benchmark] Error in getEnrichmentTaskStatus:', error)
    return {
      success: false,
